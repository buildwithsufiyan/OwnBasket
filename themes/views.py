...
def template_mapping(request, theme_id):
    ...
    if request.method == "POST":
        ...
        # --- PHASE 3: Trigger the conversion ---
        run_template_conversion(theme)
        messages.info(request, "Automatic conversion process has been started.")
        
        return redirect("themes:template_converter_review", theme_id=theme.id)
